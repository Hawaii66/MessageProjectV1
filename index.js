...
const db = monk("localhost/allMessages");
    ...
    channels.find().then(channelsCurrent => {
        ...
        for (let i = 0; i < channelsCurrent.length; i++) {
            if (channelsCurrent[i].channel !== undefined) {
                if (channelsCurrent[i].channel == req.body.newChannelText) {
                    alreadyExist = true;
                }
            }
        }

        if (alreadyExist || !isValidChannelName(req.body.newChannelText)) {
            res.json("ERROR: ALREAY EXISTS");
        } else {
            const toInsert = {
                channel: req.body.newChannelText.toString(),
                created: new Date()
            }
